...
static int32_t ulong2int(unsigned long val)
{
    if (val > INT_MAX)
    {
        qWarning() << "This conversion from ulong to int may result in data lost, because the value exceeds INT_MAX. Before: " << val << ", After: " << INT_MAX;
        return INT_MAX;
    }
    return static_cast<int32_t>(val);
}
...
static unsigned long int2ulong(int32_t val)
{
    if (val < 0)
    {
        qWarning() << "This conversion from int to ulong may result in data lost, because the value is less than 0. Before: " << val << ", After: " << 0;
        return 0;
    }
